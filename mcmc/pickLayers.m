...
% calculate correlation of harmonics
for i = 1:length(stat)
    for j = 1:length(stat)
        C(i,j) = xcorr(fun(i,:),fun(j,:),0,'coeff');
    end
end

% Cluster analysis (clusters until minimum cluster size is 3)
coi = C-min(C(:));
coi = coi./max(coi(:));
dismat = 1-coi;
...
% There should be no groups with less than three stations
if nmin == 2
    Ngroups = Ngroups-1;
    groups = cluster(Z,'MaxClust',Ngroups);
    groupsut = unique(groups);
...
% sort groups for station order (sorted for profile coordinates)
for i = 1:length(groupsut)
    temp = sum(coi(groups==groupsut(i),groups==groupsut(i)),1);
    statoi = stat(groups==groupsut(i));
    [~,I] = max(temp);
    indxn(i) = indx(strcmp(stat,statoi{I}));
end
[~,I] = sort(indxn);
groupsn = groups;
jjn = [];
for i = 1:max(groupsut)
    groupsn(groups==groupsut(i)) = I(i);
end
for i = 1:length(groupsut)
    temp = indx(groupsut(i)==groupsn);
    temp = sort(temp);
    jjn = [jjn temp];
end
indxn = sort(indxn);
% find indices for boundaries between groups
temp = diff(groupsn);
ig = find(temp>0);
end
